...
$FoldersToRemove = Get-ChildItem .\ -include bin,obj,TestResults -Recurse   | where {$_ -notmatch '_tools' -and $_ -notmatch '_build'} | foreach {$_.fullname}

# recursively get all folders matching given includes
$AllFolders = Get-ChildItem .\ -include bin,obj,TestResults -Recurse | foreach {$_.fullname}
